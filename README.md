...
Отдельный статический обучающий хаб с шестью интерактивными уроками по префлопу и постфлопу:
...
После запуска откройте `http://127.0.0.1:4173`. Локальный сервер поддерживает те же чистые URL, что и Vercel, например `/rfi-open-position-lesson` и `/flop-checkraise-lesson`.
