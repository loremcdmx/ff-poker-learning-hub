...
assert.equal(data.version, "vs3bet-field-cube-20260725-v7");
...
assert.equal(data.meta.provenance.rankIntervals.rows, 19699);
assert.equal(data.meta.provenance.handCube.rows, 103288);
...
assert.match(clickhouse, /h\.preflop_raise_and_blind_made_amount_bb > 0/);
assert.match(clickhouse, /h\.amt_preflop_3bet_facing_bb \/ h\.preflop_raise_and_blind_made_amount_bb >= 2\.25/);
assert.match(clickhouse, /h\.amt_preflop_3bet_facing_bb \/ h\.preflop_raise_and_blind_made_amount_bb < 4\.5/);
assert.match(clickhouse, /face_action = 'R' AND preflop_action = 'RR' AND is_allin = 1, 'jam'/);

assert.equal(diagnostics.duplicateRows, 0);
assert.equal(diagnostics.chartCount, 480);
assert.equal(diagnostics.publicChartCount, 480);
assert.equal(diagnostics.structurallyValidChartCount, 480);
assert.equal(diagnostics.missingStructurallyValidCharts, 0);
assert.deepEqual(diagnostics.missingStructurallyValidChartKeys, []);
assert.equal(Object.keys(data.charts).length, 480);
assert.deepEqual(Array.from(data.meta.sizeBuckets), ["2.5", "3", "4"]);
assert.deepEqual(Array.from(data.meta.sourceSizeBuckets), ["2.5", "3", "4"]);
for (const key of ["novice|EP|IP|20-30|2.5", "novice|EP|IP|20-30|4"]) {
...
assert.equal(diagnostics.global.opportunities, 5958573);
...
assert(diagnostics.lastHandAt.startsWith("2026-07-21"));
...
const defaultAqsMixes = ["league1", "league2", "league3"].map((cohort) => handMix(`${cohort}|CO|IP|31-50|3`, "AQs"));
...
const filterSignatures = [
  ["league2|CO|IP|31-50|2.5", "AQs"],
  ["league2|CO|IP|31-50|3", "AQs"],
  ["league2|CO|IP|31-50|4", "AQs"],
  ["league2|HJ|OOP|20-30|4", "AQs"]
