...
--
-- Text-network raise amounts are interpreted from the action suffix, never
-- from the actor nickname. `raises X to Y` already reports the final total-to
-- commitment. The only additive exception is 888Poker's single-amount,
-- no-`to` form, where the amount is added to Hero's already-posted blind.
...
cards_and_action AS (
  SELECT
    *,
    ...
    extract(
      hero_action_line,
      '(?i)^.+?(?::)?\\s+(?:folds?|calls?|raises?|checks?)\\b(.*)$'
    ) AS hero_action_suffix,
    arrayMap(
      amount -> toFloat64OrZero(replaceAll(amount, ',', '')),
      extractAll(hero_action_suffix, '[0-9][0-9,.]*')
...
text_observations AS (
  SELECT
    ...
    if(
      network = '888Poker'
        AND length(hero_action_amounts) = 1
        AND positionCaseInsensitive(hero_action_suffix, ' to ') = 0,
      hero_action_amount + hero_posted_blind,
      hero_action_amount
    ) AS hero_commitment_amount,
