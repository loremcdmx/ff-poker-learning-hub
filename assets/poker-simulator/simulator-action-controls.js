      ...
      function renderActions(table) {
        ...
        if (table.status !== "playing") {
          ...
          if (sessionLimit > 0 && completedHands >= sessionLimit) {
            const playAgain = current.settings?.pack === "resteal-bb-demo"
              ? '<button class="table-action is-main" type="button" data-action="resteal-play-again">Сыграть ещё</button>'
              : "";
        ...
        if (facingRaiseDecision) {
          if ((finalAllInDecision || callOnlyRaiseDecision) && !shortAllInDecision) {
            return `
              <div class="client-controls is-facing-raise is-final-all-in" data-commit-action="call">
                ${timebankHtml}
          ...
          if (isAllInOnly || shortAllInDecision) {
            return `
              <div class="client-controls is-shove is-facing-raise" data-commit-action="raise-custom">
                ${timebankHtml}
                <div class="client-row">
                  ${actionButton("fold", foldLabel, "is-fold")}
          ...
          return `
            <div class="client-controls is-facing-raise" data-commit-action="raise-custom">
              ${timebankHtml}
