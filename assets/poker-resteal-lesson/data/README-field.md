...
`fold_pct` и `continue_pct` — доли в `[0,1]`. Continue включает технические
коды трекера `C` и `R`, когда первоначальный опенер продолжил после all-in Hero.
`R` не трактуется как отдельный ререйз или изоляция: в проверенном прямом
all-in-срезе он не подтверждается флагами squeeze/4-bet и в продукте всегда
объединяется с `C` в одно стратегическое решение «Продолжил».
