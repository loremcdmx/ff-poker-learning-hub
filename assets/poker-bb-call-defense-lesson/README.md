...
- A separate empirical readout comes from a frozen, aggregated FF tracker export for BB calls at 0–40, 40–70, and 70+ BB effective (`data/ff-bb-call-realization.json`). A retained 25–40 BB compatibility slice supports snapshot reconciliation. It is descriptive data about hands that were actually called, not a solver recommendation or a causal estimate.
- The empirical headline uses all FF 3–9 max tables to reach a publishable per-hand sample and says so in the UI. Exact 7-max observations are stored as a sparse subset diagnostic: they already occur once inside the 3–9 max total and are never added a second time.
- The 2026-07-17 snapshot contains 4,858,125 EV-ready calls across the three displayed stack buckets and 7,474 observed stack × size × opener × hand cells. Percentages stay hidden below N=500 and are marked preliminary below N=2,000.
