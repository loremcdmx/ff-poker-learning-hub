...
const normalizedAssetHash = (file) => createHash("sha256")
  .update(fs.readFileSync(path.join(lessonRoot, file), "utf8").replace(/\r\n/g, "\n").replace(/\r/g, "\n"))
  .digest("hex")
  .slice(0, 12);
...
assert.match(html, /Сравниваем руки с одинаковыми рангами среди уже сыгранных коллов; это описание выборки, а не рекомендация коллировать каждую руку/);
...
assert.match(html, /Реальные раздачи FF\. Сравниваем непересекающиеся ранговые группы/);
...
assert.match(dataScreenMarkup, /Как различается защита BB между уровнями/);
...
const leagueCohortSource = js.slice(js.indexOf("  function leagueCohortItems()"), js.indexOf("\n\n  function renderLeagueDefenseControls"));
assert.match(leagueCohortSource, /league2[\s\S]*?league1/);
assert.doesNotMatch(leagueCohortSource, /league3/, "overlapping rank-15 League 3 cohort is not learner-selectable");
assert.match(js, /leagueCohort:\s*"league2"/);
assert.match(js, /непересекающиеся ранги 15–18/);
...
assert.match(css, /\.league-filter-group\.is-stack \.segmented-control,[\s\S]*?\.league-filter-group\.is-size \.segmented-control \{ grid-template-columns: repeat\(3, minmax\(0, 1fr\)\); \}/);
assert.match(css, /\.league-filter-group\.is-league \.segmented-control \{ grid-template-columns: repeat\(2, minmax\(0, 1fr\)\); \}/);
...
assert.equal((html.match(/<small>Стек 58 BB<\/small>/g) || []).length, 2, "intro table subtracts BTN open plus BB blind and ante");
...
assert.match(html, /Граница первоисточника/);
assert.match(html, /не указано, что у них одинаковы стек, анте и турнирный контекст/);
assert.match(html, /не объясняй разницу между сайзингами только ценой колла/);
assert.match(css, /\.range-provenance-note \{/);
...
assert.match(sizeLadder, /Опен BTN 2 BB[\s\S]*?91,0%/);
...
assert.doesNotMatch(js, /K4o — чистый колл|Некрасивая, но зелёная/, "first coach does not reveal the answer before the click");
assert.match(js, /Проверь границу[\s\S]*?ответ зависит от позиции, сайза и клетки чарта/);
...
assert.match(html, /остановиться всегда можно кнопкой «Закончить»/);
assert.doesNotMatch(html, /кнопкой «К уроку»/);
...
assert.match(html, /Эквити и модельный порог колла/);
assert.match(js, /Это модельный порог безубыточности колла, а не наблюдаемая прибыль конкретной руки/);
assert.doesNotMatch(html, /id="realizationStackTabs"/);
assert.doesNotMatch(contentJs, /ffRealizationModel|ff-bb-call-realization\.json/);
assert.doesNotMatch(js, /ensureFfRealizationData|ffRealization|meanEvVsFoldBb|meanRealizedEquityPct|Качество данных|Реализация в сыгранных коллах/);
assert.doesNotMatch(css, /\.ff-realization/);
...
assert.match(js, /M: \{ raise: 0, call: 50 \}/);
assert.match(js, /memoryToolMarkup\("B", "Микс 3-бет \/ колл"\)/);
assert.match(js, /memoryToolMarkup\("M", "Микс колл \/ пас"\)/);
assert.match(css, /\.is-code-b\s*\{[\s\S]*?linear-gradient\(135deg, #f79a9f 0 50%, #b9dfbd 50% 100%\)/);
assert.match(css, /\.is-code-m\s*\{[\s\S]*?linear-gradient\(135deg, #b9dfbd 0 50%, #fff 50% 100%\)/);
assert.match(js, /Оба типа микса сохраняются с исходным весом 50\/50\./);
...
assert.ok(html.includes(`poker-progress/progress.js?v=${normalizedAssetHash("../poker-progress/progress.js")}`), "shared progress JS cache token");
assert.ok(html.includes(`poker-bb-call-defense-lesson/lesson.css?v=${assetHash("lesson.css")}`), "BB lesson CSS cache token");
assert.ok(html.includes(`poker-bb-call-defense-lesson/recall.js?v=${assetHash("recall.js")}`), "BB recall JS cache token");
