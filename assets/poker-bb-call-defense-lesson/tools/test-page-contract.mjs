...
assert.match(html, /32 410[\s\S]*?сопоставленных коллов на сторону/);
assert.match(html, /описание поля, не рекомендация/);
...
assert.match(html, /Наблюдаемая игра поля, не рекомендация; прямой пуш входит в 3-бет/);
...
assert.match(js, /exactObserved \+ "\. Эти раздачи уже входят в общий срез 3–9 max и второй раз не прибавляются\.<\/small><\/section>"/);
