(function () {
  ...
  function feedbackFor(correct, hand, sourceCell) {
    return {
      fold: correct === "fold"
        ? "Да. В исходной матрице " + hand + " — белая клетка: базовая линия здесь пас."
        : "Пас не совпадает с базовой матрицей методички для этого сайза и позиции.",
      call: correct === "call"
        ? "Да. В исходной матрице " + hand + " — зелёный колдколл."
  ...
  var firstSpot = spot({
    ...
    openPosition: "BTN",
    correct: "call",
    sourceCell: "колдколл 100%",
    reason: "Нужно добавить 1 BB в итоговый банк 5,5 BB, а K4o полностью лежит в зелёной части исходной матрицы BTN 2x.",
    cue: "Не оценивай руку по внешнему виду: сначала позиция, сайз и клетка в чарте."
  });

  var practiceSpots = [
    spot({ id: "k4o-btn-2", title: "Некрасивый король", hand: "K4o", cards: ["Kh", "4d"], sizeKey: "2_0", openPosition: "BTN", correct: "call", sourceCell: "колдколл 100%", reason: "Именно такие разномастные короли выглядят как пас на автопилоте, но против BTN 2x остаются в зелёной части чарта.", cue: "Главный лик урока: не выкидывай пограничную руку до того, как прочитал позицию и сайз." }),
    spot({ id: "q6o-btn-2", title: "Разномастная дама", hand: "Q6o", cards: ["Qh", "6c"], sizeKey: "2_0", openPosition: "BTN", correct: "call", sourceCell: "колдколл 100%", reason: "Q6o входит в широкую защиту против BTN 2x, хотя визуально кажется слабой.", cue: "BTN минрейзит широко; не сдвигай границу колла до красивых карт." }),
    spot({ id: "q6o-btn-3", title: "Та же рука против 3x", hand: "Q6o", cards: ["Qh", "6c"], sizeKey: "3_0", openPosition: "BTN", correct: "fold", sourceCell: "фолд 100%", reason: "При 3x цена выросла до 26,7%, и Q6o уже белая клетка.", cue: "Сравни с Q6o против 2x: меняется только сайз, но решение переворачивается." }),
    spot({ id: "j6o-btn-2", title: "Разномастный валет", hand: "J6o", cards: ["Jh", "6d"], sizeKey: "2_0", openPosition: "BTN", correct: "call", sourceCell: "колдколл 100%", reason: "J6o — ещё одна частая пропущенная зелёная клетка против BTN 2x.", cue: "Тренируем узнавание до автоматизма: увидел клетку — нажал колл." }),
    spot({ id: "t6o-btn-2", title: "Десятка с шестёркой", hand: "T6o", cards: ["Th", "6c"], sizeKey: "2_0", openPosition: "BTN", correct: "call", sourceCell: "колдколл 100%", reason: "T6o полностью лежит в колле против BTN 2x в исходной матрице.", cue: "Слабый вид руки не отменяет цену 18,2% и широкий диапазон BTN." }),
    spot({ id: "96o-btn-2", title: "Девятка с шестёркой", hand: "96o", cards: ["9h", "6d"], sizeKey: "2_0", openPosition: "BTN", correct: "call", sourceCell: "колдколл 100%", reason: "96o остаётся зелёной клеткой: связанность помогает руке реализовывать эквити.", cue: "Карты ближе друг к другу защищаются лучше, чем низкие разнесённые offsuit." }),
    ...
    spot({ id: "tt-btn-2", title: "Сильная пара", hand: "TT", cards: ["Th", "Ts"], sizeKey: "2_0", openPosition: "BTN", correct: "raise", sourceCell: "3-бет 100%", reason: "В исходной матрице TT целиком уходит в розовую часть, а не в колл.", cue: "Урок про колл не означает, что весь деф нужно коллировать." }),
    ...
    spot({ id: "ajo-ep-2", title: "Минрейз из ранней", hand: "AJo", cards: ["Ah", "Jc"], sizeKey: "2_0", openPosition: "EP", correct: "call", sourceCell: "колдколл 100%", reason: "Даже против EP рука заметно выше нижней границы колла на минрейз.", cue: "Позиция сужает защиту, но не превращает сильную руку в пас." }),
    ...
    spot({ id: "76s-btn-3", title: "Сьютовый коннектор", hand: "76s", cards: ["7h", "6h"], sizeKey: "3_0", openPosition: "BTN", correct: "call", sourceCell: "колдколл 100%", reason: "Несмотря на цену 26,7%, 76s остаётся зелёной клеткой против BTN в исходной матрице.", cue: "Смотри на связку сайз + позиция + конкретная рука." }),
    spot({ id: "k2s-btn-25", title: "Сьютовый король", hand: "K2s", cards: ["Ks", "2s"], sizeKey: "2_5", openPosition: "BTN", correct: "call", sourceCell: "колдколл 100%", reason: "Против широкого BTN K2s сохраняется в зелёной части диапазона.", cue: "Поздняя позиция рейзера расширяет защиту." }),
    spot({ id: "55-mp-3", title: "Пара против 3x", hand: "55", cards: ["5h", "5c"], sizeKey: "3_0", openPosition: "MP", correct: "call", sourceCell: "колдколл 100%", reason: "55 остаются зелёной клеткой даже при более дорогом колле.", cue: "Не оверфолди готовые пары только из-за сайза." }),
    spot({ id: "qts-hj-2", title: "Две связанные карты", hand: "QTs", cards: ["Qd", "Td"], sizeKey: "2_0", openPosition: "HJ", correct: "call", sourceCell: "колдколл 100%", reason: "QTs хорошо играет после флопа и полностью лежит в колле.", cue: "Сьютовость и связанность помогают реализовывать эквити." }),
    ...
    spot({ id: "87s-co-25", title: "Сьютовый коннектор против CO", hand: "87s", cards: ["8d", "7d"], sizeKey: "2_5", openPosition: "CO", correct: "call", sourceCell: "колдколл 100%", reason: "87s остаётся зелёной клеткой против CO 2,5x.", cue: "Играбельные сьютовые руки реализуют больше эквити." }),
    ...
    spot({ id: "jts-hj-25", title: "JTs против HJ", hand: "JTs", cards: ["Jh", "Th"], sizeKey: "2_5", openPosition: "HJ", correct: "call", sourceCell: "колдколл 100%", reason: "JTs остаётся в зелёной части против HJ 2,5x.", cue: "Постфлоп-играбельность важна для реализации." }),
    spot({ id: "99-btn-3", title: "Сильная пара против BTN", hand: "99", cards: ["9h", "9s"], sizeKey: "3_0", openPosition: "BTN", correct: "raise", sourceCell: "3-бет 100%", reason: "99 — розовая клетка в исходной матрице BTN 3x.", cue: "Сильная рука может предпочитать 3-бет даже против крупного сайза." })
  ];

  window.PokerBbCallData = Object.freeze({
    version: "bb-call-defense-alpha-20260712-v2",
    sourcePdf: "/Users/loremcdmx/Downloads/Telegram Desktop/часть методички для аишки (1) (2).pdf",
    wisdomTranscript: "/Users/loremcdmx/Downloads/Telegram Desktop/покерный урок 2.txt",
