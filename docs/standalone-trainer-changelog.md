...
- В `AGENTS.md` добавлены правила для будущих правок:
  ...
  - перед показом узкого среза обновлять его точной выгрузкой FF MCP; при
    структурном нуле скрывать или отключать детализацию, а не подменять её
    агрегатом или модельной оценкой.
...
- Узкие срезы показывают только точные наблюдаемые счётчики из выгрузки FF;
  структурно отсутствующие комбинации не выводятся как заполненные данные.
...
- Уточняется наполнение data slices:
  - узкие размерные и hand-level срезы получают ближайший уже загруженный
    агрегат, а не пустую/слабую карточку;
  - несуществующая детализация убирается из UX вместо текстовой заглушки;
...
- Для всех chart/data routes нужно поддерживать один стиль:
  ...
  - узкие срезы агрегируются до наполненного уровня или не выводятся.
