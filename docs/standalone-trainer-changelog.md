...
- `Released` — изменение входит в production-историю за 2026-07-17—2026-07-25.
...
- Первая раздача курса теперь показывает 98s после опена CO и позиционного
  3-бета BTN вместо очевидной премиальной пары JJ.
- Главная линия синхронизирована с чартом этого узла: колл 79,4%; пас отмечен
  как слишком тайтовое решение, а 4-бет оставлен редким допустимым миксом.
- Карты, вопрос, пояснения каждого действия и контрактный тест переведены на
  один и тот же спот, чтобы на экране не оставалось логики прежней руки.

## 2026-07-26 — защита против 3-бета: вопрос перед раздачей

Статус: `Released`.

- Блок `Твой ход` перенесён из нижней части стола в отдельную компактную
  карточку над раздачей: задача видна до просмотра действий и не теряется под
  кнопками.
- После ответа тот же блок показывает результат на прежнем месте над столом,
  поэтому обратная связь не меняет геометрию кнопок действий.
...
- Убраны повторный вводный абзац и служебная шапка над первым интерактивным
  столом: название спота и подсказка про расположение действий больше не
  дублируют уже видимые элементы раздачи.
- После блока `Что тренируем` пользователь сразу видит стол; его контейнер
  получил самостоятельную доступную подпись без ссылки на удалённый заголовок.

## 2026-07-26 — защита против 3-бета: компактная кнопка колла

Статус: `Released`.

- На первом столе и во всех следующих префлоп-ситуациях кнопка показывает
  короткое действие `Колл` без повторения уже видимой на столе суммы доплаты.
...
- Первый интерактивный стол стал ниже, а карты, плашки игроков и фишки —
  крупнее: важные объекты читаются без пустого фиолетового поля вокруг них.
- Крайние места сдвинуты внутрь сукна, а ставка Hero разведена с банком и
  карманными картами. Кнопки действий и вопрос теперь попадают в первый экран
  ноутбука без изменения функциональной общей оболочки симулятора.

## 2026-07-25 — одинаковый визуал нашего чарта и игры поля

Статус: `Released`.

- Матрица `Как играет поле` переведена на тот же визуальный контракт, что
  `Наш чарт`: высота ячейки показывает встречаемость руки, цвет и нижняя
  полоса — смесь паса, колла, 4-бета и пуша.
- Для выбранной руки рядом показываются две одинаково оформленные строки
  `Наш чарт` и `Поле`, поэтому рекомендацию и реальные решения FF можно
  сравнить по каждому действию без переключения вкладок.

## 2026-07-25 — одинаковые размеры 3-бета в чарте и данных поля

Статус: `Released`.

- Во вкладке `Как играет поле` старые абсолютные бакеты `до 6 BB / 6–8 BB /
  8–10 BB / 10+ BB` заменены теми же относительными размерами, что и в нашем
  чарте: `2,5x / 3x / 4x`.
- Размер считается по фактическому отношению полученного 3-бета к исходному
  опену. Поэтому один и тот же переключатель теперь имеет одинаковый смысл в
  рекомендации, таблице фолдов и наблюдаемой матрице 169 рук.
- Полевой куб пересобран из `5 958 573` решений FF за
  `2025-07-01—2026-07-21`: `480` точных срезов, по `169` рук в каждом
  доступном чарте. Переключение размера меняет и сводку по позициям/стекам, и
  выбранную hand-level матрицу.

## 2026-07-25 — три префлоп-тренажёра: читаемые диапазоны и единая геометрия

Статус: `Released`.

- В общей оболочке тренажёров `SB без входов`, `Против одного рейзера по
  позициям` и `SB против одного рейзера` закреплены единые цвета действий,
  подписи и responsive-проверки.
- В тренажёре игры против одного рейзера по позициям наложение двух когорт
  заменено двумя синхронными матрицами 13×13: `Первая лига` и `Ранги 15–18`.
  Все 169 рук видны отдельно; жёлтая рамка отмечает только комбинации, у
  которых различается основная линия.
- На широких экранах увеличена глубина стола тренажёра по позициям, чтобы
  карты, ставки и места игроков не собирались в центре. На ноутбуках и
  мобильных сохранена компактная раскладка.
- Общий browser-smoke проверяет три маршрута на широком, ноутбучном и
  мобильном экранах: отсутствие обрезаний, переполнения, наложений и
  возвращения старого нечитаемого overlay-чарта.
...
- Добавлена проверка всех 72 целевых комбинаций
  ...
  тайтовее 3x.
- После перехода полевого куба на реальные размеры `2,5x / 3x / 4x`
  полностью сверены 480 точных срезов и 81 120 hand-level клеток:
...
- Убрали конечный размер пака: практика продолжает выдавать новые ситуации, пока игрок сам не завершит сессию.
- Упростили стартовую настройку до трёх понятных режимов — все ситуации, в позиции и без позиции.
- Точные фильтры по позиции, стеку и размеру 3-бета перенесли в компактную необязательную настройку.
- Убрали техническую строку с количеством доступных раздач и заменили кнопку запуска на стабильное «Начать практику».
